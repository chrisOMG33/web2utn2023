...
class ProductosController {
  ...
  static add = async (req: Request, resp: Response) => {
    ...
    try {
      ...
      if(!nombre) {
        return resp.status(400).json({ mensaje: "Debe indicar un nombre" });
      }
      if(!precio) {
        return resp.status(400).json({ mensaje: "Debe indicar el precio" });
      }
      if(precio<0) {
        return resp.status(400).json({ mensaje: "Debe indicar un precio mayor que 0" });
      }
      if(!stock) {
        return resp.status(400).json({ mensaje: "Debe indicar el precio" });
      }  
      ...
      if(pro) {
        return resp.status(400).json({ mensaje: "El producto ya existre en la base de datos" });
      }
